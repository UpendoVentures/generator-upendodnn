...
namespace <%= fullNamespace %>.Data
{
    ...
    public class <%= extensionName %>Context: DbContext
    {
        public <%= extensionName %>Context()
            : base("name=SiteSqlServer")
        {
            Database.SetInitializer<<%= extensionName %>Context>(null);
        }
        ...
        {
            string moduleQualifier = "<%= extensionName %>_";
