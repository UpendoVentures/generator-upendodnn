<<%= openDirective %> Control Language="C#" AutoEventWireup="true" CodeBehind="Login.ascx.cs" Inherits="<%= fullNamespace %>.Login" <%= closeDirective %>>
<<%= openDirective %> Register TagPrefix="dnn" Assembly="DotNetNuke" Namespace="DotNetNuke.UI.WebControls" <%= closeDirective %>>
<<%= openDirective %> Register TagPrefix="dnn" Namespace="DotNetNuke.Web.UI.WebControls.Internal" Assembly="DotNetNuke.Web" <%= closeDirective %>>
...
<dnn:DnnScriptBlock runat="server">
    <script type="text/javascript">
        ...
        (function ($, Sys) {
            ...
            const actionLinks = $('a[id^="dnn_ctr<<%= closeDirective %>=ModuleId > Null.NullInteger ? ModuleId.ToString() : ""<%= closeDirective %>>_Login_Login_DNN"]');
            ...
            $(document).ready(function () {
                $(document).on('keydown', '.dnnLoginService', function (e) {
                    if ($(e.target).is('input:text,input:password') && e.keyCode === 13) {
                        var $loginButton = $('#dnn_ctr<<%= closeDirective %>=ModuleId > Null.NullInteger ? ModuleId.ToString() : ""<%= closeDirective %>>_Login_Login_DNN_cmdLogin');
